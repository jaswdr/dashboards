...
with tab4:
    ...
    comment("Pclass has strong negative correlation with `Survived`, "
...
for index, col in enumerate(cols):
    ...
    fig = px.histogram(df, x=col, color="Survived", text_auto=True)
...
for col in onehot_encoding_columns:
    ...
    result = onehot_encoder.fit_transform(df[col].values.reshape(-1,
                                                                 1)).toarray()
...
fig = px.imshow(confussion_matrix, text_auto=True, title="Confusion Matrix")
fig.update_layout(
